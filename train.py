...
def train_net(net, args, ema_net=None, fold_idx=0):
    ...
    # Move criterion to MPS device
    device = torch.device('mps' if torch.backends.mps.is_available() else 'cpu')
    criterion = nn.CrossEntropyLoss(weight=torch.tensor(args.weight).to(device).float())
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_epoch(trainLoader, net, ema_net, optimizer, epoch, writer, criterion, criterion_dl, scaler, args, device)
...
def train_epoch(trainLoader, net, ema_net, optimizer, epoch, writer, criterion, criterion_dl, scaler, args, device):
    ...
    for i, inputs in enumerate(trainLoader):
        img, label = inputs[0].to(device), inputs[1].long().to(device)
        ...
        if args.amp:
            with torch.autocast(device_type='mps', dtype=torch.float16):
                result = net(img)
                loss = calculate_loss(result, label, criterion, criterion_dl, args)
...
def calculate_loss(result, label, criterion, criterion_dl, args):
    loss = 0
...
def init_network(args):
    device = torch.device('mps' if torch.backends.mps.is_available() else 'cpu')
    net = get_model(args, pretrain=args.pretrain).to(device)

    ema_net = None
    if args.ema:
        ema_net = get_model(args, pretrain=args.pretrain).to(device)
...
if __name__ == '__main__':
    ...
    for fold_idx in range(args.k_fold):
        ...
        # Initialize the network and move to the appropriate device
