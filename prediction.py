...
def prediction(model_list, tensor_img, args):
    ...
    with torch.no_grad():
        # CHANGED: Ensure tensor_img is float32 on CPU before sending to MPS
        tensor_img = tensor_img.float().to("mps")
...
def preprocess(itk_img, target_spacing, args):
    '''
    This function performs preprocessing to make images consistent with training,
    e.g. spacing resample, redirection, etc.
    '''
    if args.dimension == '3d':
        ...
        if (x_spacing, y_spacing) != target_spacing:
            itk_img = ResampleXYZAxis(itk_img, 
                                      (target_spacing[0], target_spacing[1], z_spacing), 
    ...
    # Example scaling/clipping (ACDC-like normalization)
    ...
    # CHANGED: cast to float32 so we don't run into float64 -> MPS errors
    tensor_img = torch.from_numpy(np_img.astype(np.float32))
...
def init_model(args):
    ...
    for ckp_path in args.load:
        ...
        # If you want to load checkpoint trained with a previous code version, do:
...
def get_parser():
    ...
    parser.add_argument('--load', type=parse_model_list, default=False, 
                        help='Path(s) to trained model checkpoint(s). '
                             'Use "," to load multiple for ensemble.')
    parser.add_argument('--img_path', type=str, default=False, 
                        help='Directory of images to be predicted')
    parser.add_argument('--save_path', type=str, default='./result/', 
                        help='Path to save predicted label')
    parser.add_argument('--target_spacing', type=parse_spacing_list, 
                        default='1.0,1.0,1.0', 
                        help='Spacing used for training (x,y,z for 3D, x,y for 2D)')
